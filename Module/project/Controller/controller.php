...
class controller extends dashboard implements \Controller\IController {
    ...
    function __construct() {
        ...
        if ($this->getAction() != "index") {
            if (!\Module\project\Model\Project::GetCurentProject()) {
...
//            var_dump(\Module\project\Model\Project::DecodeData($this->getParam()[0]));
    ...
    function clear() {
        ...
        $_Model = new \Module\project\Model\Controller($ModelController->GetById($Datatoken->Id));
        $ControllerData = new \Module\project\Model\ControllerData();
        $where = "`ControllerId` = '{$_Model->Id}'";
        $ControllerData->DeleteRowByWhere($where);
    ...
    public function create() {
        ...
        if (\Module\project\Model\ControllerForm::onSubmit()) {
            try {
                ...
                if (!isset($proj["PId"]))
                    $proj["PId"] = \Module\project\Model\Project::GetCurentProject(true)->Id;
                $_Model["PId"] = $proj["PId"];
                $_Model["Lat"] = $proj["Lat"];
                $_Model["Lon"] = $proj["Lon"];
                $_Model["Ip"] = $proj["Ip"];
...
//            \Application\redirectTo::Url($_SERVER["HTTP_REFERER"]);
    ...
    public function datacontrollers1() {
